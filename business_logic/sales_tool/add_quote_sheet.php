...
$font= $_POST['field_4_add'];
$size= $_POST['field_5_add'];
...
try
{
	...
	$STH = $DBH->prepare("UPDATE quote_sheet SET best = :best, better = :better, good = :good, font = :font, size = :size WHERE company_id=:company_id");

	$STH->bindParam(':best', $best);
	$STH->bindParam(':better', $better);
	$STH->bindParam(':good', $good);
	$STH->bindParam(':font', $font);
