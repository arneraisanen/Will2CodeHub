...
try
{
	...
	$STH = $DBH->prepare("SELECT id, material_id, quantity FROM proposal WHERE company_id='$company_id' AND proposal_id='$proposal_id' AND type_field='gold'");
	$STH->execute();
...
$series_entries = '<table id="materials_proposal_add_table_gold" class="table table-hover">
    <thead>
      <tr>
        <th>#</th>
        <th>ID</th>
