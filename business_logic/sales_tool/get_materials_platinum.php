...
$series_entries = '    
  <table style="display:none;" id="materials_table_1" class="table table-hover">
    <thead>
      <tr>
        <th>#</th>
        <th>ID</th>
...
while($row = $STH->fetch())
{
	$series_entries .= '<tr style="display:none;" id="material_row_id_' . $row["id"] . '">
        ...
        <td><button type="button" onclick="choose_materials_table(1, ' . $row["id"] . ');" class="btn btn-sm btn-success">Add</button></td></tr>';
