...
try
{
	...
	$STH = $DBH->prepare("SELECT id, name, trade_name, rate FROM subcontractors WHERE id='$id'");
	$STH->execute();
	...
	while($row = $STH->fetch())
	{
		...
		$profile_edit = <<<EOD
			...
			<div class="row">
				<div class="col-sm-6">
					<div class="email-form">
						...
						<div class="req">
							<input value="$trade_name" id="trade_name" name="title" class="school-name" required="required" data-trigger="focus" type="text" value=""/>
						</div>
					</div>
				</div>
			</div>
			...
			<div class="cancel-submit">
				<ul>
					<li><a href="#" onclick="location.href='subcontractor_management.php'">Cancel</a></li>
					<li>
						<button id="registerBtn" type="button" onclick="subcontractors_update($id);">Update</button>
					</li>
				</ul>
			</div>
...
{
	$profile_edit = 'Subcontractor (ID: ' . $id . ') was deleted<br /><br /><div class="cancel-submit">
				<ul>
					<li><a href="#"  onclick="location.href=\'subcontractor_management.php\'">Back to Subcontractors page</a></li>
				</ul>
			</div>';
	...
	try
	{
		...
		$STH = $DBH->prepare("DELETE FROM subcontractors WHERE id = :id");
  ...
  <body>
	<?php include $_SERVER['DOCUMENT_ROOT']."/business_logic/admin_tool/insert_admin_menu.php"; ?>
        ...
        <div id="fullscreen_block" class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
         <h2 class="sub-header">Edit Subcontractors</h2>
