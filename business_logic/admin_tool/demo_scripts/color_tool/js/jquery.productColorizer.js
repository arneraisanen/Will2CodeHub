		...
		return this.each(function() {
			...
			$(swatches).click(function(e) {
				...
				if($(mask).attr('role')) {
					...
					$('[id^="tt-mask"]').each(function () {
						$(this).css({"background-color": color}, 1000);
						if($.browser.msie) {
							color = $(this).attr('rel');
							var colors = color.split(",");
							color = colorToHex(colors);
							$(this).css({'background': 'transparent', 'zoom': 1, 'filter': 'progid:DXImageTransform.Microsoft.gradient(startColorstr=#80' + color + ',endColorstr=#80' + color + ')'});
						}
					})
				}
			});
			...
			$(swatches).hover(function(e) {
				...
				var offsetY = $(this).offset().top - $(this).parent().offset().top + 50;
