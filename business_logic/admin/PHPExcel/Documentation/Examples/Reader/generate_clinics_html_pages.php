...
foreach(glob('docdir/*.*') as $file) 
{
	...
	$inputFileName = 'docdir/' . basename($file);
	$objReader = PHPExcel_IOFactory::createReader($inputFileType);
	$objPHPExcel = $objReader->load($inputFileName);
	$writer = PHPExcel_IOFactory::createWriter($objPHPExcel, 'HTML');
	$writer->writeAllSheets();
	$writer->save('html_pages/' . basename($file, '.xls') . '.html');
}

$ajax_response .= '<br /><br />';
$ajax_response_start = 'The following ' . $count . ' files were found under html_pages/ and read (see tabs below for details):<br /><br />';
